...
cb_app_status cb_search_insert(cb_search *search, unsigned int character) {
	if (!search || character < 32 || character > 126) return CB_APP_INVALID;
	if (search->query_length >= CB_SEARCH_QUERY_MAX) return CB_APP_INVALID;
	search->query[search->query_length++] = (char)character;
